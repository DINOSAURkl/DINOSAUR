...
class Graph:
    ...
    def function3(self, node1, node2) -> list:
        ans = []
    ...
    def function4(self, content):
        ...
        for i in range(len(content) - 1):
            if content[i] not in nodes or content[i + 1] not in nodes:
                continue
            result = self.function3(content[i], content[i + 1])
            if result != []:
...
if __name__ == '__main__':
    ...
    # ans = G.function3('explore', 'life')
    # for item in ans:
    #     print(item)

    # G.function4("Life in new worlds")

    # G.function5('life', 'new')
    # G.function5('explore', 'life')
    # G.function5('worlds')
